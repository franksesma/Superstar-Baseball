...
function VFXModule.TransferEffects(sourceContainer: Instance, targetPart: BasePart, duration: number?)
	if not sourceContainer then
		warn("TransferEffects: sourceContainer is nil.")
		return
	end
	if not targetPart or not targetPart:IsA("BasePart") then
		warn("TransferEffects: Invalid targetPart.")
		return
	end

	-- Try to find a BasePart inside the container
	local sourcePart
	if sourceContainer:IsA("BasePart") then
		sourcePart = sourceContainer
	else
		for _, child in ipairs(sourceContainer:GetDescendants()) do
	...
	if not sourcePart then
		warn("TransferEffects: No BasePart found inside sourceContainer.")
		return
	end

	for _, attachment in ipairs(sourcePart:GetChildren()) do
		if attachment:IsA("Attachment") then
			local clonedAttachment = attachment:Clone()
			clonedAttachment.Parent = targetPart

			for _, effect in ipairs(clonedAttachment:GetChildren()) do
				if effect:IsA("ParticleEmitter") then
					effect:Emit(effect:GetAttribute("EmitCount") or 40)
			...
			if duration and typeof(duration) == "number" then
				game:GetService("Debris"):AddItem(clonedAttachment, duration)
			end
		end
	end
end
