...
local flowers = script.Parent:WaitForChild("Flowers", 2):GetChildren()
...
while not script.Parent:GetAttribute("Free") do
	task.wait()
end

print (script.Parent:GetAttribute("Free"))

for _, wall in script.Parent.Walls:GetChildren() do
	print ("walk through")
	wall.CanCollide = false
end

for _, object in script.Parent:GetDescendants() do
	if not object:IsDescendantOf(script.Parent.Walls) and object:IsA("BasePart") then
